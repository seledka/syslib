...
namespace SYSLIB
{
    static void shaHashBlock(SHA1_CTX *ctx)
    {
        int t;
...
SYSLIBFUNC(BOOL) hash_CalcSHA1(const LPBYTE lpData,DWORD dwLen,LPBYTE lpHash)
{
    BOOL bRet=false;
    do
    {
        ...
        if (!SYSLIB_SAFE::CheckParamWrite(lpHash,20))
            break;

        SHA1_CTX ctx;

        SYSLIB::SHA1Init(&ctx);
        SYSLIB::SHA1Update(&ctx, lpData, dwLen);
        SYSLIB::SHA1Finish(&ctx, lpHash);
        bRet=true;
    }
    while (false);
    return bRet;
